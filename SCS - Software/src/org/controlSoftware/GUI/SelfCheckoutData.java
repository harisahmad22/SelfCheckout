...
//	private TouchScreenSoftware touchScreenSoftware;
	...
	public enum StationState {
		...
		// Item has been scanned, enter processing state to block new scans/additions
		// until item has been put in bagging area.
		PROCESSING_SCAN,

		// Membership related states. Need only enable disable card reader for SCAN
		// state.
		...
		// State for when user has paid total due, and system is waiting for them to
		// take their items
		// Once Scale observer detects weight = 0 when in CLEANUP state, state will
		// change to WELCOME
		CLEANUP,
		...
		// State to handle checking if change is needed, dispensing and updating the
		// receipt data
		// then displaying GUI window asking user if they would like a receipt, button
		// listeners for this window
		// will call the hardware's printer print() method and cut_paper() method if the
		// user chooses to get their receipt
		// After they have made a decision, check if there is still money to be paid. If
		// so, move to NORMAL state, otherwise
		// go to WELCOME state
...
//		ADD_MEMBERSHIP,
		...
		// General post-checkout state, check if more payment is needed, if so move to
		// NORMAL state.
		...
		// This State can only be entered if system is in WELCOME state. (Not being used
		// by a customer)
		// Later could have all system's methods except startupStation() not work if the
		// state == INACTIVE
		INACTIVE,

		// State that is entered after an item is scanned/added via lookup
		// Will only be exited once Bagging area scale handler detects weight on scale
		// == expected weight
		// At which point system returns to NORMAL mode
		WAITING_FOR_ITEM,

		//// State that is entered after an item is scanned/added via lookup/PLU
		// Will only be exited once Scanning area scale handler detects weight on scale
		// == expected weight
		// At which point system returns to NORMAL mode
		WAITING_FOR_SCANNED_ITEM,

		// State that is entered once a weight issue is detected, can also be entered if
		// weight is invalid after system begins waiting for item to be put down
		WEIGHT_ISSUE,

		// State that allows system to exit from WEIGHT_ISSUE state, will update
		// expected weights to current scale weight
		ATTENDANT_OVERRIDE,

		// State to inform GUI to display keypad for user to enter in their payment
		// amount
		PARTIAL_PAYMENT_KEYPAD,

		// State to inform user that their gift card does not have enough funds to
...
//			System.out.println("WAITING FOR ITEM");
		...
		case WEIGHT_ISSUE:
			...
			// TODO GUI Window will have a skip bagging button that will force a state
			// change to
...
//			
		case PRINT_RECEIPT_PROMPT:
			...
			// dispense, and update receipt. Then inform GUI to display receipt prompt
			// window
			// GUI listeners will handle when the user makes a choice. If they choose to
			// print
			// then listener will call hardware methods to print. After, they will check if
			// money still
...
//			stationSoftware.getTouchScreenSoftware().askToPrintReceipt(stationSoftware.getReceiptHandler());
		...
		case PAY_CASH:
			...
			// state will change to the PRINT_RECEIPT_PROMPT state, which will determine if
			// system
			// needs to give change, ask to print receipt, then go to the CLEANUP or NORMAL
			// state
...
//			station.baggingArea.disable();
	...
	public PLUProductDatabase getPLUDatabase() {
		return PLU_Product_Database;
	}

	public BarcodedProductDatabase getBarcodedDatabase() {
