...
public class BarcodedProductDatabase {
	...
	public BarcodedProductDatabase(ArrayList<BarcodedProduct> objects) {
		...
		if (!objects.isEmpty() || objects != null) {
			for (BarcodedProduct p : objects) {
				database.put(p.getBarcode(), p);
			}
		}
	...
	public void addBarcodedProductToDatabase(Barcode barcode, double weightInGrams, String description,
			BigDecimal price) {
	...
	public void removeBarcodedProductFromDatabase(Barcode barcode) {
